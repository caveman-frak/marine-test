...
public class InMemoryRepository<T, ID> implements JpaRepository<T, ID> {
	...
	public InMemoryRepository(final Function<T, ID> extractor,
		...
		this(extractor, inserter, generator,
				Arrays.stream(entities).collect(Collectors.toMap(extractor, e -> e)));
	}
	...
	@Override
	@SuppressWarnings("unchecked")
	public <S extends T> @NonNull Page<S> findAll(@NonNull Example<S> example, @NonNull Pageable pageable) {
	...
	@Override
	@SuppressWarnings("unchecked")
	public <S extends T, R> @NonNull R findBy(@NonNull Example<S> example, Function<FluentQuery.FetchableFluentQuery<S>, R> queryFunction) {
		return queryFunction.apply((FluentQuery.FetchableFluentQuery<S>) findOne(example).orElseThrow());
	}
}
